...
const DetailRequest1 = () => {
  ...
  useEffect(() => {
       const fetchchanges1 = async () => {
     try {
       let jwtToken = GetJWT();
       let response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v2/request/detail/${idRequest}`, {
         headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
       });
       if (response.data && response.data.changes1) {
         setchanges1(response.data.changes1);
       ...
       jwtToken = GetJWT();
       response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v2/request/site/detail/${idRequest}`, {
         headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
       });
       ...
       setLoading(false); // Ensure loading is set to false after both API calls
     }
   };
  ...
  const handleSubmit = async (status, notes) => {
    try {
      ...
      await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/v2/request/verify/${idRequest}`, changeRequest, {
  ...
  return (
    ...
    {siteDetails && (
      ...
      {isDetailsOpen && siteDetails && (
        ...
        <p>{siteDetails.cluster_name || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">Regional Name: </p>
        <p>{siteDetails.regional_name || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">Witel Name: </p>
        <p>{siteDetails.witel_name || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">STO Name: </p>
        <p>{siteDetails.sto_name || "N/A"}</p>
        </div>
        <div className="mb-4">
        ...
        <p>{siteDetails.remark || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">Status OSP: </p>
        <p>{siteDetails.status_osp || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">IHLD: </p>
        <p>{siteDetails.ihld || "N/A"}</p>
        </div>
        <div className="mb-4">
        <p className="font-semibold">Catuan ID: </p>
        <p>{siteDetails.catuan_id || "N/A"}</p>
        </div>
