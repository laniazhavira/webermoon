...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let jwtToken = GetJWT();
        let response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v2/data/table`,  {
        ...
        jwtToken = GetJWT();
        response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v2/data/priority`,  {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional: selectedRegion, filterWitel: selectedWitel, filterSto, type: "sto" }
        });
  ...
  const handleStoClick = (sto) => {
    navigate("/tabel1/site", { state: { selectedWitel, selectedSto: sto } });
  };
    ...
    <div className="bg-gray-100  min-h-[calc(100vh-111px)] min-w-screen p-8">
      ...
      <div className="max-w-[98%] mx-auto">
        ...
        <div className="flex items-center gap-3 mb-5 relative">
          <button 
            className="px-5 py-2.5  bg-gray-700 text-white rounded-lg flex items-center gap-2 hover:bg-red-600 transition shadow-md"
              ...
              {Object.entries(payload).map(([key, value]) => {
                  ...
                  return (
                    <tr key={key} className="odd:bg-gray-50 hover:bg-gray-100 transition duration-150 text-center">
                      <td className={`border border-gray-200 px-4 py-3 ${key === "Total" ? ' bg-gray-700 text-white' : ''}`}>
                        <button
                      ...
                      {[...Array(end - start)].map((_, i) => (
                        <td key={i} className={`border border-gray-200 px-4 py-3 ${key === "Total" ? ' bg-gray-700 text-white font-bold' : ''}`}>{value.values[i + start]}</td>
                      ))}

                      {(selectedSection === 'ALL' || selectedSection === 'PRIORITY') &&
                        priority[key] &&
                        ["P1", "P2", "P3", "P4"].map((p, i) => (
                          <td key={`priority-${key}-${i}`} className={`border border-gray-200 px-4 py-3 ${key === "Total" ? ' bg-gray-700 text-white font-bold' : ''}`}>
                            {priority[key][p] || 0}
                          </td>
                        ))
                      }
