    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response_chart = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v3/data/chart`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional, status: "survey" }
        });
        ...
        <div className="flex gap-8">
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      ...
                      <th className="bg-red-500 text-white border-b-2 border-red-600 px-6 py-3 text-center" colSpan="2">SURVEY</th>
                    </tr>
                  ...
                  <tbody>
                    {payload && Object.entries(payload).map(([key, value]) => (
                      <tr key={key} className={`hover:bg-gray-50 ${key === "Total" ? 'font-bold bg-gray-50' : ''}`}>
                        ...
                        <td className="border-b border-gray-200 px-6 py-4 text-center">{value.values[2]}</td>
                        <td className="border-b border-gray-200 px-6 py-4 text-center">{value.values[3]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="w-3/5 bg-white rounded-xl shadow-lg overflow-hidden flex">
            <div className="p-6 ml-4">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Kurva S Status Survey csf Edge OTN</h2>
              <div className="overflow-x-auto">
