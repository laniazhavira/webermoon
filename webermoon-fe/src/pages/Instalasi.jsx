...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  // Description data
  const descriptions = {
    "Plan Instalasi": "Rencana pelaksanaan instalasi",
    "Realisasi Instalasi": "Status pelaksanaan instalasi yang sudah dilakukan",
    "Progress": "Persentase kemajuan dari proses instalasi",
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/table`, {
        ...
        const response_chart = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/chart`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional, status: "instalasi" }
        });
        SetJWT(response_chart.headers.authorization);
        
        // Transform data for chart
        if (response_chart.data.result) {
          setChartData(transformChartData(response_chart.data.result, "Plan Instalasi", "Realisasi Instalasi"));
        }
      ...
      <div className="max-w-[100%] mx-auto">
        ...
        <div className="flex gap-4 mb-8 relative">
          ...
          {dropdownVisible && (
            ...
            >
              <button 
                className="block w-full text-left px-5 py-3 hover:bg-gray-300 hover:text-white transition border-b"
              ...
              {Object.entries(listRegional).map(([key, value]) => (
                  <button
                    key={key}
                    className="block w-full text-left px-5 py-3 hover:bg-gray-300 hover:text-white transition"
          ...
          <div className="relative">
            <button 
              className="px-6 py-2.5 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition shadow-md flex items-center gap-2"
        ...
        <div className="flex gap-8">
          ...
          <div className="w-2/5 bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Instalasi Data</h2>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center">Treg</th>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center" colSpan="2">INSTALASI</th>
                    </tr>
                    <tr className="bg-gray-700 text-white">
                      <th className="border-b-2 border-gray-600 px-6 py-3"></th>
                      <th className="border-b-2 border-gray-600 px-6 py-3">Plan</th>
                      <th className="border-b-2 border-gray-600 px-6 py-3">Realisasi</th>
                    </tr>
                  </thead>
                  <tbody>
                    {payload && Object.entries(payload).map(([key, value]) => (
                      <tr key={key} className={`hover:bg-gray-100 ${key === "Total" ? 'font-bold bg-gray-100' : ''}`}>
                        <td className="text-center border-b border-gray-300 px-6 py-4">{key}</td>
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[6]}</td>
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[7]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="w-3/5 bg-white rounded-xl shadow-lg overflow-hidden flex">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Kurva S Status Instalasi CSF Mini OLT</h2>
              <div className="overflow-x-auto">
