...
const Site = () => {
  ...
  const sections = {
    ...
    STATUS_OSP: { name: "Status Osp", cols: 1 }
  };
...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/site`, {
  ...
  const renderTableHeaders = () => {
    ...
    if (selectedSection === 'ALL' || selectedSection === 'STATUS_OSP') {
      headers.push(
        <th key="status-osp" className="border px-4 py-3" rowSpan="2">STATUS OSP</th>
      );
    }
  ...
  const handleSiteClick = (idSite) => {
    navigate(`/tabel/site/detail/${idSite}`);
  };

  const addSite = () => {
    navigate("/tabel/site/add", { state: { selectedWitel, selectedSto } });
  };
        ...
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-3 relative">
            ...
            {dropdownVisible && (
              ...
              >
                <button
                  className="block w-full text-left px-5 py-3 hover:bg-gray-200 hover:text-gray-800 transition border-b"
                ...
                {Object.entries(listSite).map(([key, value]) => (
                  <button
                    key={key}
                    className="block w-full text-left px-5 py-3 hover:bg-gray-200 hover:text-gray-800 transition"
          ...
          {/* Add Site Button */}
          <button
            className="ml-auto px-5 py-2.5 bg-gray-700 text-white rounded-lg flex items-center gap-2 hover:bg-gray-800 transition shadow-md"
            onClick={() => addSite()}
          >
            + Add Site
          </button>
              ...
              <tbody>
                {Object.entries(payload).map(([key, value]) => {
                  return (
                    <tr
                      key={key}
                      className="odd:bg-gray-50 hover:bg-gray-100 transition duration-150 text-center"
                    >
                      ...
                      {(selectedSection === "ALL" || selectedSection === "STATUS_OSP") && (
