...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  // Description data
  const descriptions = {
    "Drop": "Indikator untuk pelepasan atau pemberhentian layanan",
    "Relokasi": "Proses pemindahan atau pengalihan lokasi layanan",
    "Not Yet": "Status untuk layanan yang belum diproses atau ditindaklanjuti",
    "Progress": "Persentase kemajuan dari seluruh proses",
  };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data.result) {
          const chartData = Object.entries(response.data.result)
            .filter(([key]) => key !== "Total")
            .map(([key, value]) => ({
              name: key,
              drop: value.values[0],
              relokasi: value.values[1]
            }));
      ...
      <div className="max-w-[100%] mx-auto">
        ...
        <div className="flex gap-8">
          ...
          <div className="w-2/5 bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Not Yet Data</h2>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center">Treg</th>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center" colSpan="2">NOTYET</th>
                    </tr>
                    <tr className="bg-gray-700 text-white">
                      <th className="border-b-2 border-gray-600 px-6 py-3"></th>
                      <th className="border-b-2 border-gray-600 px-6 py-3">Drop</th>
                      <th className="border-b-2 border-gray-600 px-6 py-3">Relokasi</th>
                    </tr>
                  </thead>
                  <tbody>
                    {payload && Object.entries(payload).map(([key, value]) => (
                      <tr key={key} className={`hover:bg-gray-100 ${key === "Total" ? 'font-bold bg-gray-100' : ''}`}>
                        ...
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[0]}</td>
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[1]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="w-3/5 bg-white rounded-xl shadow-lg overflow-hidden flex">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Kurva S Status Not Yet CSF Mini OLT </h2>
              <div className="overflow-x-auto">
                <LineChart width={600} height={400} data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" label={{ value: 'Region', position: 'bottom' }} />
            ...
            <div className='flex flex-col items-center justify-center'>
                ...
                <h2 className='font-bold w-2/3 text-center'>Khusus Drop tidak dapat di-track tanggalnya</h2>
            </div>
          </div>
        </div>
