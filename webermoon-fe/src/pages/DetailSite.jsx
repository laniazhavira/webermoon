  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/site/${idSite}`,  {
  ...
  return (
    <div className="bg-gray-100  min-h-[calc(100vh-111px)] min-w-screen p-8">
      ...
      <div className="bg-gradient-to-r from-gray-500 to-gray-700 text-white p-6 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-center">{payload.cluster_name}</h2>
        <p className="text-center text-lg mt-2">Priority: {payload.status_osp}</p>
      ...
      <div className="mt-6 bg-white shadow-lg rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-gray-500 font-semibold">IHLD:</span>
            <span className="text-lg">{payload.ihld}</span>
          </div>

          <div className="flex flex-col">
            <span className="text-gray-500 font-semibold">Catuan ID:</span>
      ...
      {/* Remark Section */}
      {payload.remark && (
        <div className="mt-6 bg-gray-300 border-l-4 border-gray-500 text-red-700 p-4 rounded-lg">
      ...
      <div className="flex justify-center mt-6">
        ...
        {role === "mitra" && (
            <button
            onClick={() => navigate(`/tabel/site/request/${idSite}`)}
