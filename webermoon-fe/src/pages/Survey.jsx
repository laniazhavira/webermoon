...
const Survey = () => {
  ...
  // Description data
  const descriptions = {
    "Plan Survey": "Rencana pelaksanaan survey",
    "Realisasi Survey": "Status pelaksanaan survey yang sudah dilakukan",
    "Progress": "Persentase kemajuan dari proses survey",
  };
  ...
  useEffect(() => {
    ...
    try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v3/data/table`, {
        ...
        const response_chart = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v3/data/chart`, {
            headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
            params: { filterRegional, status: "survey" } // Ensure this is correct
        });
        SetJWT(response_chart.headers.authorization);

        // Transform data for chart
        if (response_chart.data.result) {
            setChartData(transformChartData(response_chart.data.result, "Plan Survey", "Realisasi Survey"));
        }
      ...
      <div className="max-w-[100%] mx-auto">
        ...
        <div className="flex gap-8">
          ...
          <div className="w-2/5 bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Survey Data</h2>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-700 px-6 py-3 text-center">Treg</th>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-700 px-6 py-3 text-center" colSpan="2">SURVEY</th>
                    </tr>
                    <tr className="bg-gray-700 text-white">
                      <th className="border-b-2 border-gray-700 px-6 py-3"></th>
                      <th className="border-b-2 border-gray-700 px-6 py-3">Plan</th>
                      <th className="border-b-2 border-gray-700 px-6 py-3">Realisasi</th>
                    </tr>
                  </thead>
                  <tbody>
                    {payload && Object.entries(payload).map(([key, value]) => (
                      <tr key={key} className={`hover:bg-gray-100 ${key === "Total" ? 'font-bold bg-gray-100' : ''}`}>
                        ...
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[2]}</td>
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[3]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="w-3/5 bg-white rounded-xl shadow-lg overflow-hidden flex">
            <div className="p-6 ml-4">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Kurva S Status Survey CSF Mini-OLT</h2>
              <div className="overflow-x-auto">
