...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  // Description data
  const descriptions = {
    "Plan Delivery": "Rencana pelaksanaan delivery",
    "Realisasi Delivery": "Status pelaksanaan delivery yang sudah dilakukan",
    "Progress": "Persentase kemajuan dari proses delivery",
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response_table = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/table`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional, type: "regional" }
        });
        setPayload(response_table.data.result);

        const response_chart = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/chart`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional, status: "delivery" }
        });
        SetJWT(response_chart.headers.authorization);
        
        // Transform data for chart
        if (response_chart.data.result) {
          setChartData(transformChartData(response_chart.data.result, "Plan Delivery", "Realisasi Delivery"));
        }

        // Set listRegional only on first render
        if (firstRender.current) {
          setListRegional(getRegionals(response_table.data.result));
      ...
      <div className="max-w-[100%] mx-auto">
        ...
        <div className="flex gap-8">
          ...
          <div className="w-2/5 bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Delivery Data</h2>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center">Treg</th>
                      <th className="bg-gray-700 text-white border-b-2 border-gray-600 px-6 py-3 text-center" colSpan="2">DELIVERY</th>
                    </tr>
                  ...
                  <tbody>
                    {payload && Object.entries(payload).map(([key, value]) => (
                      <tr key={key} className={`hover:bg-gray-100 ${key === "Total" ? 'font-bold bg-gray-100' : ''}`}>
                        ...
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[4]}</td>
                        <td className="border-b border-gray-400 px-6 py-4 text-center">{value.values[5]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="w-3/5 bg-white rounded-xl shadow-lg overflow-hidden flex">
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Kurva S Status Delivery CSF Mini OLT</h2>
              <div className="overflow-x-auto">
                <LineChart width={600} height={400} data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" label={{ value: 'DateTime', position: 'bottom' }} />
