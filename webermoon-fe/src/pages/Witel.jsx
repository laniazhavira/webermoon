...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  const handleWitelClick = (witel) => {
    navigate("/tabel/sto", { state: { selectedRegion, selectedWitel: witel } });
  };
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-gray-700 text-white">
              <th className="border px-4 py-3" rowSpan="2">
                Witel
              </th>
              {renderTableHeaders()}
            </tr>
            <tr className="bg-gray-700 text-white">{renderSubHeaders()}</tr>
          </thead>
          <tbody>
            {Object.entries(payload).map(([key, value]) => {
              ...
              return (
                <tr
                  key={key}
                  className="odd:bg-gray-50 hover:bg-gray-200 transition duration-150 text-center"
                >
                  <td
                    className={`border border-gray-200 px-4 py-3 ${
                      key === "Total" ? "bg-gray-700 text-white font-bold" : ""
                    }`}
                  >
                    <button
                      onClick={() => handleWitelClick(value.id)}
                      className={`border border-gray-200 px-4 py-3 ${
                        key === "Total" ? "pointer-events-none" : ""
                      }`}
                      disabled={key === "Total"}
                    >
