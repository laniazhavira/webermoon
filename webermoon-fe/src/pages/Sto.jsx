...
const Sto = () => {
  ...
  const { selectedRegion, selectedWitel } = location.state || {};
  ...
  const [listSto, setListSto] = useState([]);
...
const formattedDate = `${datePart}, ${timePart}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let jwtToken = GetJWT();
        let response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/table`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional: selectedRegion, filterWitel: selectedWitel, filterSto, type: "sto" }
        });
        ...
        if (firstRender.current) {
          setListSto(getStos(response.data.result));
          firstRender.current = false; // Mark as executed
        }

        jwtToken = GetJWT();
        response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/data/priority`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
          params: { filterRegional: selectedRegion, filterWitel: selectedWitel, filterSto, type: "sto" }
        });
  ...
  const handleFilterChange = (sto, id) => {
    setSelectedSto(sto);
    setFilterSto(id);
    setDropdownVisible(false);
  };
  ...
  const handleStoClick = (sto) => {
    navigate("/tabel/site", { state: { selectedWitel, selectedSto: sto } });
  };
  ...
  const renderSubHeaders = () => {
    ...
    if (selectedSection === 'ALL' || selectedSection === 'INTEGRASI') {
      subHeaders.push(
        <th key="drop2" className="border px-4 py-3">Drop</th>,
        <th key="relokasi2" className="border px-4 py-3">Relokasi</th>
      );
    }
    if (selectedSection === 'ALL' || selectedSection === 'PRIORITY') {
      subHeaders.push(
    ...
    <div className="bg-gray-100  min-h-[calc(100vh-111px)] min-w-screen p-8">
      ...
      <div className="max-w-[98%] mx-auto">
        ...
        <div className="flex items-center gap-3 mb-5 relative">
          <button
            className="px-5 py-2.5 bg-gray-700 text-white rounded-lg flex items-center gap-2 hover:bg-gray-800 transition shadow-md"
            onClick={toggleDropdown}
          >
            Sto Filter: {selectedSto || 'All'} <span className="text-sm">▼</span>
          </button>
          ...
          {dropdownVisible && (
            ...
            >
              ...
              {Object.entries(listSto).map(([key, value]) => (
                <button
                  key={key}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-700 text-white">
                  <th className="border px-4 py-3" rowSpan="2">Sto</th>
                  {renderTableHeaders()}
                </tr>
                <tr className="bg-gray-700 text-white">
                  {renderSubHeaders()}
                ...
                {Object.entries(payload).map(([key, value]) => {
                  ...
                  return (
                    <tr key={key} className="odd:bg-gray-50 hover:bg-gray-100 transition duration-150 text-center">
                      <td className={`border border-gray-300 px-4 py-3 ${key === "Total" ? 'bg-gray-700 text-white' : ''}`}>
                        <button
                          onClick={() => handleStoClick(value.id)}
                      ...
                      {[...Array(end - start)].map((_, i) => (
                        <td key={i} className={`border border-gray-300 px-4 py-3 ${key === "Total" ? 'bg-gray-700 text-white font-bold' : ''}`}>{value.values[i + start]}</td>
                      ))}

                      {(selectedSection === 'ALL' || selectedSection === 'PRIORITY') &&
                        priority[key] &&
                        ["P1", "P2", "P3", "P4"].map((p, i) => (
                          <td key={`priority-${key}-${i}`} className={`border border-gray-300 px-4 py-3 ${key === "Total" ? 'bg-gray-700 text-white font-bold' : ''}`}>
                            {priority[key][p] || 0}
                          </td>
                        ))
                      }
