...
const DetailRequest = () => {
  ...
  useEffect(() => {
    const fetchChanges = async () => {
      try {
        let jwtToken = GetJWT();
        let response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/request/detail/${idRequest}`,  {
        ...
        jwtToken = GetJWT();
        response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/request/site/detail/${idRequest}`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
        });
