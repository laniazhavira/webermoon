...
const DetailRequest1 = () => {
  ...
  useEffect(() => {
    const fetchChanges = async () => {
      try {
        let jwtToken = GetJWT();
        let response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v4/request/detail/${idRequest}`,  {
        ...
        jwtToken = GetJWT();
        response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v4/request/site/detail/${idRequest}`, {
          headers: { Authorization: `Bearer ${jwtToken}`, "Cache-Control": "no-cache" },
        });
  ...
  const handleSubmit = async (status, notes) => {
    try {
      ...
      await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/request/verify/${idRequest}`, changeRequest, {
