...
router.get("/site/detail1/:idRequest", authorizeUser, async(req, res) => {
    try {
        ...
        const clusterSql = `
            SELECT 
                ...
                c.plan_integrasi, c.integrasi, c.is_drop, c.type_olt, c.merk_olt, c.prioritas
            FROM clusters3 c
            JOIN sto3 s ON c.sto_id = s.id AND c.witel_id = s.witel_id
            JOIN witel3 w ON s.witel_id = w.id
            JOIN regional3 r ON w.regional_id = r.id
...
router.get("/detail1/:idRequest", authorizeUser, async(req, res) => {
    try {
...
router.patch("/verify1/:idRequest", authorizeAdmin, async(req, res) => {
    try {
        ...
        if (flowStatus === "Approved") {
            ...
            sql = `UPDATE clusters3 SET ${setClause} WHERE id = ?`
            ...
            const sqlUpdateStatus = `UPDATE clusters3 SET status = ? WHERE id = ?`;
...
router.post("/site1/:idCluster/add", authorizeMitra, async(req, res) => {
    try {
        ...
        const [cluster] = await db.query(`SELECT * FROM clusters3 WHERE id = ?`, [idCluster]);
        if (cluster.length === 0) {
