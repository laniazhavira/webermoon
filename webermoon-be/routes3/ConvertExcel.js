...
async function getOrInsertSto(connection, stoName, witelId, cache) {
    ...
    try {
        // Check if exists
        const [existing] = await connection.query(
            `SELECT id FROM sto3 WHERE name = ? AND witel_id = ?`, [stoName, witelId]
        );
        if (existing.length > 0) {
        ...
        const [maxResult] = await connection.query(
            `SELECT MAX(id) AS maxId FROM sto3 WHERE witel_id = ?`, [witelId]
        );
        const newId = (maxResult[0].maxId || 0) + 1;

        // Insert new sto record
        await connection.query(
            `INSERT INTO sto3 (id, name, witel_id) VALUES (?, ?, ?)`, [newId, stoName, witelId]
        );
...
router.post('/excel-upload', authorizeAdmin, fileUpload(), async (req, res) => {
    ...
    try {
        ...
        // Clear clusters3 table before new inserts
        await connection.query('DELETE FROM clusters3');
        ...
        for (const row of rows) {
            ...
            const regionalId = await getOrInsert(connection, 'regional3', regionalName, regionalCache);
            if (!regionalId) {
                throw new Error(`Failed to get or insert regional: ${regionalName}`);
            }

            // Insert or get witel id with foreign key regional_id
            const witelId = await getOrInsert(connection, 'witel3', witelName, witelCache, { regional_id: regionalId });
            if (!witelId) {
            ...
            // Insert record into clusters3 table with all fields
            await connection.query(
                `INSERT INTO clusters3 
