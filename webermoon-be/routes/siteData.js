...
// For curve sql query
function generateQuery(columnName, filterRegional, params) {
    return `
        SELECT 
        ...
        WHERE 
            status IN ('Plan Survey', 'Realisasi Survey')  -- Include both Plan and Realisasi
...
router.get("/priority", authorizeGuest, async(req, res) => {
    try {
        ...
        let sql = `
            SELECT 
                ${valueTable}, status_osp as priority, COUNT(*) AS count 
            FROM 
                clusters c
            JOIN sto s
                ON c.sto_id = s.id AND c.witel_id = s.witel_id
        ...
        sql += ` GROUP BY ${valueTable}, status_osp ORDER BY ${valueTable}, status_osp`;
...
router.get("/site", authorizeGuest, async(req, res) => {
    try {
        ...
        let sql = `
            SELECT 
                id, name, is_drop, mitra, remark, status_osp, plan_survey, survey, plan_delivery, delivery, plan_instalasi, instalasi, plan_integrasi, integrasi, ihld, catuan_id
            FROM 
                clusters c
            WHERE 
                witel_id = ? AND sto_id = ?`;
...
router.get("/site/:idSite", authorizeGuest, async(req, res) => {
    try {
        ...
        let sql = `
            SELECT 
                c.id as cluster_id, c.name as cluster_name, r.name as regional_name, w.name as witel_name, s.name as sto_name, is_drop, mitra, remark, status_osp, plan_survey, survey, 
                plan_delivery, delivery, plan_instalasi, instalasi, plan_integrasi, integrasi, ihld, catuan_id
            FROM 
                clusters c
            JOIN sto s
                ON c.sto_id = s.id AND c.witel_id = s.witel_id
...
router.get("/site/form-detail/:idSite", authorizeMitra, async(req, res) => {
    try {
        ...
        let sql = `
            SELECT 
                c.id as cluster_id, is_drop, status_osp, plan_survey, survey, 
                plan_delivery, delivery, plan_instalasi, instalasi, plan_integrasi, integrasi, ihld, catuan_id, remark
            FROM
                clusters c
            JOIN sto s
                ON c.sto_id = s.id AND c.witel_id = s.witel_id
    ...
    try {
        ...
        let sql = `
            INSERT INTO clusters 
                (name, sto_id, witel_id, mitra, status, remark, status_osp, plan_survey, survey, plan_delivery, delivery, plan_instalasi, instalasi,
                plan_integrasi, integrasi, is_drop, ihld, catuan_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        let params = [name, sto_id, witel_id, mitra, status, remark, status_osp, plan_survey, survey, plan_delivery, delivery, plan_instalasi, instalasi,
            plan_integrasi, integrasi, is_drop, ihld, catuan_id
        ];
