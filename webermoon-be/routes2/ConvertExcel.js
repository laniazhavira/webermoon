...
async function getOrInsertSto(connection, stoName, witelId, cache) {
    ...
    try {
        // Check if exists
        const [existing] = await connection.query(
            `SELECT id FROM sto2 WHERE name = ? AND witel_id = ?`, [stoName, witelId]
        );
        if (existing.length > 0) {
        ...
        const [maxResult] = await connection.query(
            `SELECT MAX(id) AS maxId FROM sto2 WHERE witel_id = ?`, [witelId]
        );
        const newId = (maxResult[0].maxId || 0) + 1;

        // Insert new sto record
        await connection.query(
            `INSERT INTO sto2 (id, name, witel_id) VALUES (?, ?, ?)`, [newId, stoName, witelId]
        );
...
router.post('/excel-upload', authorizeAdmin, fileUpload(), async (req, res) => {
    ...
    try {
        ...
        // Clear clusters2 table before new inserts
        await connection.query('DELETE FROM clusters2');
        ...
        for (const row of rows) {
            ...
            const regionalId = await getOrInsert(connection, 'regional2', regionalName, regionalCache);
            if (!regionalId) {
                throw new Error(`Failed to get or insert regional: ${regionalName}`);
            }

            // Insert or get witel id with foreign key regional_id
            const witelId = await getOrInsert(connection, 'witel2', witelName, witelCache, { regional_id: regionalId });
            if (!witelId) {
            ...
            // Insert record into clusters2 table with all fields
            await connection.query(
