...
router.get("/site/detail/:idRequest", authorizeUser, async (req, res) => {
    try {
        ...
        const clusterSql = `
            ...
            WHERE c.id = (SELECT cluster_id FROM logs2 WHERE id = ?)`;
...
router.patch("/verify/:idRequest", authorizeAdmin, async (req, res) => {
    try {
        ...
        const [log] = await db.query(`SELECT flow_status as status FROM logs2 WHERE id = ?`, [idRequest]);
        ...
        if (flowStatus === "Approved") {
            ...
            sql = `UPDATE clusters2 SET ${setClause} WHERE id = ?`;
            ...
            const sqlUpdateStatus = `UPDATE clusters2 SET status = ? WHERE id = ?`;
...
router.post("/site/:idCluster/add", authorizeMitra, async (req, res) => {
    try {
        ...
        const [clusterOld] = await db.query(`SELECT * FROM clusters2 WHERE id = ?`, [idCluster]);
        ...
        const [logResult] = await db.query(`
            INSERT INTO logs2 (cluster_id, requested_by, requested_at, flow_status)
